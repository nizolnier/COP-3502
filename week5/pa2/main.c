...
// function that receives an array of queues, fills them up with input data, and returns the amount of customers
int loadCustomers(queue *lines)
{
...
int getNextCustomers(queue *lines, int curTime)
{
...
int main(void)
{
    ...
    for (int i = 0; i < testCases; i++)
    {
        ...
        int customers = loadCustomers(lines);
        ...
        for (int j = 0; j < customers; j++)
        {
            ...
            int index = getNextCustomers(lines, curTime);

            // if the index is negative, i just break the loop (just to be safe)
            if (index == -1)
            {
                break;
            }
